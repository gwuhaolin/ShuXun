 ...
 * 更新微信AccessToken
 */
AV.Cloud.define('updateWechatAccessToken', function (request, response) {
    WechatAPI.updateAccessToken();
    if (WechatAPI.AccessToken) {
        response.success();
    } else {
        response.error();
    }
...
AV.Cloud.define('saveWechatImageToUsedBook', function (request, response) {
    ...
    var accessToken = WechatAPI.AccessToken;
    if (!accessToken) {
        response.error('获取AccessToken失败');
        return;
    }
