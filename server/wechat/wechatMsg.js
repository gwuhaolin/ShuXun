...
function searchBookFromDouban(keyword, res) {
        ...
        DoubanBook.searchBooks(keyword, null, 0, 10, null).done(function (json) {
            ...
            if (total > 0) {//找到了对于的书
                ...
                for (var i = 0; i < books.length && i < 9; i++) {//最多9本书
                    ...
                    var bookUrl = 'http://www.ishuxun.cn/wechat/#/book/one-book/?isbn13=' + books[i].isbn13;
                    re.push({
                        title: title,
                        image: books[i].image,
                        url: bookUrl
                ...
                if (total > 9) {//因为微信最多可以显示10本,当有的书大于9本时为用户提供显示更多
                    re.push({
                        ...
                        url: 'http://www.ishuxun.cn/wechat/#/book/search-list?keyword=' + keyword
                    });
                }
            }
    ...
    searchBook(keyword).done(function (books) {
        if (books.length > 0) {
            ...
            res.reply('没有找到和 ' + keyword + ' 相关的书,来<a href="http://www.ishuxun.cn/wechat/#/book/search-list/">这里试试看</a>');
        }
    }).fail(function (err) {
        console.error(err);
        res.reply('');
